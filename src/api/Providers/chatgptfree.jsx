export const ChatGPTFreeProvider = "ChatGPTFreeProvider";
...
export const getChatGPTFreeResponse = async function* (chat, options) {
  // get nonce and post_id
  let _post_id, _nonce;
  ...
  let result = _response.match(/data-post-id="([0-9]+)"/);
  if (result) {
    _post_id = result[1];
  }
  else {
    throw new Error("No post_id found");
  }

  result = _response.match(/data-nonce="(.*?)"/);
  if (result) {
    _nonce = result[1];
  } else {
